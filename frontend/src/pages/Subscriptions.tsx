...
export default function Subscriptions() {
  ...
  const [cycleFilter, setCycleFilter]     = useState<'all' | 'weekly' | 'monthly' | 'yearly'>('all');
  const [upgradeLimit, setUpgradeLimit]   = useState<'subscriptions' | null>(null);
    ...
    <div className="space-y-6">
      ...
      {subs.length > 0 && (
        <div className="glass border border-border rounded-2xl px-5 py-3 flex items-center justify-between">
          <span className="text-text-muted text-sm">{subs.length} {t('nav.subscriptions').toLowerCase()}</span>
          <span className="font-bold tabular-nums">{fmt.format(totalMonthly)}<span className="text-text-muted font-normal text-sm">/mo</span></span>
        </div>
      )}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
          {[...Array(3)].map((_, i) => <div key={i} className="h-40 bg-surface-hover rounded-2xl" />)}
        </div>
      ) : error ? (
        <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 flex items-center gap-3" role="alert">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          ...
          >
            <h3 id="confirm-cancel-title" className="text-xl font-bold mb-2">{t('subscriptions.cancelSubscription')}</h3>
            <p className="text-text-muted text-sm mb-6">{t('subscriptions.cancelConfirm')}</p>
      ...
      {/* Upgrade Prompt */}
      {upgradeLimit && (
        <UpgradePrompt limitType={upgradeLimit} onClose={() => setUpgradeLimit(null)} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div
        ...
        style={{ overscrollBehavior: 'contain' }}
      >
        <h2 id="add-sub-title" className="text-2xl font-bold mb-4">{t('subscriptions.addSubscription')}</h2>
        {error && (
          <div className="p-3 mb-4 rounded-xl bg-red-500/10 border border-red-500/50 text-red-400 text-sm" role="alert" aria-live="polite">
            {error}
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="sub-name" className="block text-sm text-text-muted mb-1">{t('subscriptions.serviceName')}</label>
            <input
              id="sub-name" required type="text"
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <label htmlFor="sub-cost" className="block text-sm text-text-muted mb-1">{t('subscriptions.cost')}</label>
              <input
                id="sub-cost" required type="number" step="0.01" min="0"
                value={form.cost}
            ...
            <div className="flex-1">
              <label htmlFor="sub-cycle" className="block text-sm text-text-muted mb-1">{t('subscriptions.billingCycle')}</label>
              <select
                id="sub-cycle"
          ...
          <div>
            <label htmlFor="sub-date" className="block text-sm text-text-muted mb-1">{t('subscriptions.nextBillingDate')}</label>
            <input
              id="sub-date" required type="date"
          ...
          <div>
            <label htmlFor="sub-remind" className="block text-sm text-text-muted mb-1">{t('subscriptions.remindDays')}</label>
            <input
              id="sub-remind" type="number" min="1"
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {submitting ? t('subscriptions.saving') : t('subscriptions.addSubscriptionBtn')}
            </button>
          </div>
        </form>
      </motion.div>
...
function SubscriptionCard({
  ...
  const diffDays = Math.ceil((nextBillDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-end mt-6">
        <div>
          <p className="text-3xl font-bold tabular-nums">{fmt.format(sub.cost)}</p>
          <p className={`text-sm mt-1 flex items-center gap-1 ${
            isOverdue ? 'text-gray-400' : isUrgent ? 'text-red-400 font-medium' : 'text-text-muted'
          }`}>
            {isUrgent && !isOverdue && <AlertCircle size={14} aria-hidden="true" />}
            {isOverdue
