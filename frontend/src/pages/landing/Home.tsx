...
const STEPS = [
  { num: '01', title: 'Create a Free Account', desc: 'Sign up in under 60 seconds — no credit card required.' },
  { num: '02', title: 'Add Your Data',          desc: 'Upload receipts, log subscriptions, or add expenses. Starts tracking instantly.' },
  { num: '03', title: 'Get Full Control',        desc: 'View analytics, set alerts, and cut the costs that no longer serve you.' },
];

const TESTIMONIALS = [
  { name: 'Minh T.', role: 'Freelance Designer', quote: 'FTD saved me $340/year by showing me subscriptions I completely forgot about. Worth every second to set up.' },
  { name: 'Sarah K.', role: 'Product Manager',   quote: 'Finally a tool where I can see all my receipts AND track expenses in one place. The warranty alerts alone are gold.' },
  { name: 'Duy N.', role: 'Software Engineer',   quote: 'Switched from 3 separate apps to FTD. The analytics dashboard is exactly what I needed to understand my spending.' },
];
...
export default function LandingHome() {
  return (
    <div className="overflow-x-hidden">
          ...
          <dl className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {STATS.map(stat => (
              <motion.div key={stat.label} {...fadeUp}>
                ...
                <dd className="text-3xl font-black gradient-text tabular-nums">{stat.value}</dd>
              </motion.div>
            ))}
          </dl>
        </div>
      ...
      {/* ── Features Grid ─────────────────────────────────────── */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24" id="features" aria-labelledby="features-heading">
        <motion.div className="text-center mb-16" {...fadeUp}>
          <h2 id="features-heading" className="text-4xl font-black mb-4 text-wrap-balance">Everything You Need</h2>
          <p className="text-text-muted text-lg max-w-xl mx-auto">Six powerful modules designed to give you complete financial awareness.</p>
        </motion.div>
      ...
      {/* ── How it Works ──────────────────────────────────────── */}
      <section className="bg-surface/40 border-y border-border py-24" aria-labelledby="how-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-16" {...fadeUp}>
            <h2 id="how-heading" className="text-4xl font-black mb-4 text-wrap-balance">Up & Running in 3 Steps</h2>
            <p className="text-text-muted text-lg max-w-xl mx-auto">No complex setup. No lengthy onboarding. Just open and go.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {STEPS.map((step, i) => (
              ...
              >
                <span className="text-6xl font-black gradient-text mb-5" aria-hidden="true">{step.num}</span>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24" aria-labelledby="testimonials-heading">
        <motion.div className="text-center mb-16" {...fadeUp}>
          <h2 id="testimonials-heading" className="text-4xl font-black mb-4 text-wrap-balance">Trusted by Thousands</h2>
          <div className="flex items-center justify-center gap-1 mb-2" aria-label="5 out of 5 stars">
            {[...Array(5)].map((_, i) => <Star key={i} size={18} className="text-amber-400 fill-amber-400" aria-hidden="true" />)}
          </div>
          ...
          {TESTIMONIALS.map((t, i) => (
            ...
            >
              <div className="flex gap-0.5 mb-4" aria-label="5 stars">
                {[...Array(5)].map((_, j) => <Star key={j} size={14} className="text-amber-400 fill-amber-400" aria-hidden="true" />)}
              </div>
              <blockquote className="text-text-muted leading-relaxed mb-5 text-sm">"{t.quote}"</blockquote>
              <figcaption>
      ...
      {/* ── Pricing Teaser ────────────────────────────────────── */}
      <section className="border-y border-border bg-surface/40 py-24" aria-labelledby="pricing-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div className="text-center mb-12" {...fadeUp}>
            <h2 id="pricing-heading" className="text-4xl font-black mb-4 text-wrap-balance">Simple, Transparent Pricing</h2>
            <p className="text-text-muted text-lg max-w-xl mx-auto">Start free forever. Upgrade when you're ready for advanced features.</p>
          </motion.div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {/* Free */}
            <motion.div {...fadeUp} className="p-8 rounded-2xl glass border border-border flex flex-col">
              <h3 className="text-xl font-bold mb-1">Free</h3>
              <div className="text-4xl font-black mb-2">$0<span className="text-lg font-normal text-text-muted">/mo</span></div>
              <p className="text-text-muted text-sm mb-6">Everything you need to get started.</p>
              <ul className="space-y-3 flex-1 mb-8">
                {['Up to 50 receipts', '10 subscriptions', 'Expense tracking', 'Email alerts'].map(f => (
                  <li key={f} className="flex items-center gap-2 text-sm">
                    <CheckCircle2 size={16} className="text-green-400 flex-shrink-0" aria-hidden="true" /> {f}
                  </li>
                ))}
              </ul>
              <Link to="/register" className="w-full text-center px-6 py-3 rounded-xl border border-border hover:bg-surface-hover transition-colors font-semibold">
                Start Free
              </Link>
            </motion.div>

            {/* Pro */}
            <motion.div {...fadeUp} transition={{ delay: 0.1 }} className="p-8 rounded-2xl border-2 border-primary bg-primary/5 flex flex-col relative overflow-hidden">
              <div className="absolute top-4 right-4 px-3 py-1 bg-primary text-white text-xs font-bold rounded-full">Popular</div>
              <h3 className="text-xl font-bold mb-1">Pro</h3>
              <div className="text-4xl font-black mb-2">$4.99<span className="text-lg font-normal text-text-muted">/mo</span></div>
              <p className="text-text-muted text-sm mb-6">For power users who want everything.</p>
              <ul className="space-y-3 flex-1 mb-8">
                {['Unlimited receipts', 'Unlimited subscriptions', 'Advanced analytics', 'Priority alerts', 'Export to CSV', 'Priority support'].map(f => (
                  <li key={f} className="flex items-center gap-2 text-sm">
                    <CheckCircle2 size={16} className="text-primary flex-shrink-0" aria-hidden="true" /> {f}
                  </li>
                ))}
              </ul>
              <Link to="/register" className="w-full text-center px-6 py-3 rounded-xl bg-primary hover:bg-primary-dark text-white font-bold transition-colors shadow-lg shadow-primary/25">
                Start 14-Day Trial
              </Link>
            </motion.div>
          </div>

          <motion.p className="text-center text-text-muted text-sm mt-8" {...fadeUp} transition={{ delay: 0.2 }}>
            <Link to="/pricing" className="text-primary hover:underline">Compare all features →</Link>
          </motion.p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        <motion.div {...fadeUp} className="relative p-12 rounded-3xl border border-primary/20 bg-primary/5 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-primary/10 via-transparent to-violet-500/10 pointer-events-none" aria-hidden="true" />
          ...
          <p className="text-text-muted text-lg mb-8 max-w-xl mx-auto relative z-10">
            Join 12,000+ users who already have complete clarity on their finances.
          </p>
