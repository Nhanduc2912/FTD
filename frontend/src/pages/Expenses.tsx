...
export default function Expenses() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [expRes, sumRes] = await Promise.all([
        api.get<PaginatedResponse<IExpense>>(
          `/expenses?year=${y}&month=${m}&page=${page}&limit=20`,
        ),
  ...
  const fetchTrend = async () => {
    try {
      const res =
        await api.get<{ month: string; total: number }[]>("/expenses/trend");
      setTrendData(
        res.data.map((d) => {
          ...
          <div className="divide-y divide-border">
            {loading ? (
              ...
              filtered.map((exp, i) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg font-bold border flex-shrink-0 ${CAT_BG[exp.category] ?? CAT_BG["Other"]}`}
                  >
                    {exp.category[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full border ${CAT_BG[exp.category] ?? CAT_BG["Other"]}`}
                    >
                      {String(
                        t(`common.categories.${exp.category}`, exp.category),
                      )}
                    </span>
                  </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          ...
          >
            ...
            >
              <h2 id="exp-modal-title" className="text-2xl font-bold mb-4">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    autoComplete="off"
                    placeholder="e.g. Lunch at Pho 24…"
                  ...
                  >
                    {EXPENSE_CATEGORIES.map((c) => (
                      <option key={c} value={c}>
                        {String(t(`common.categories.${c}`, c))}
                      </option>
                    ))}
                  </select>
                </div>
        ...
        {confirmDelete && (
          ...
          >
            <motion.div
              ...
              style={{ overscrollBehavior: "contain" }}
            >
              <h3 id="exp-del-title" className="text-xl font-bold mb-2">
